...
public class Statistics {
	 ...
	 * @param  type "city","province" o "region"
	 * @param  initialDate (Opzionale) Data iniziale per il filtraggio. Se non presente è impostata di default a "04-02-2021", ossia la data di creazione di Facebook
	 * @param  finalDate (Opzionale) Data finale per il filtraggio. Se non presente è impostata di default alla data attuale
	...
	public HashMap<String, Integer> ranking(String type, @Nullable String initialDate, @Nullable String finalDate)
			throws FileNotFoundException, JSONException, IOException, ParseException, WrongParameterException, WrongFieldException {
		...
		if(finalDate == null) finalDate = LocalDate.now().plusDays(1).format(userFormatter); //Data attuale + 1 perché altrimenti non mostra la classifica aggiornata alla giornata odierna
		LocalDate date1;
		LocalDate date2;
			...
			for (int i = 0; i < loc.size(); i++) { // scorre l'ArrayList di Location
				...
				for (Entry<Post, Location> set : hm.entrySet()) {
					LocalDate createdTime = (ZonedDateTime.parse(set.getKey().getCreatedTime(), FacebookFormatter))
							.toLocalDate();
					if (loc.get(i).getCity().equals(loc.get(j).getCity()) && createdTime.isAfter(date1)
							&& createdTime.isBefore(date2)) {
						counter++;
					}
					j++;
				}
		...
		default : throw new WrongFieldException("ERRORE! Inserisci un tipo valido tra city, province o region)");
		}
